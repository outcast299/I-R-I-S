    ...
    internal class GunLib_again 
    {
        ...
        public static GunLibData Shoot()
        {
            ...
            if (XRSettings.isDeviceActive)
            {
                ...
                data.isTriggered = ControllerInputPoller.instance.rightControllerIndexTouch > .5f;
                if (data.isShooting)
                {
                    var result =
                        Physics.Raycast(
                            GTPlayer.Instance.rightControllerTransform.position -
                            GTPlayer.Instance.rightControllerTransform.up, -GTPlayer.Instance.rightControllerTransform.up,
                ...
                if (data.isShooting)
                {
                    if (data.lockedPlayer == null)
                    {
                        var result =
                            Physics.Raycast(
                                GTPlayer.Instance.rightControllerTransform.position -
                                GTPlayer.Instance.rightControllerTransform.up,
                                -GTPlayer.Instance.rightControllerTransform.up, out var hit, float.PositiveInfinity,
                                Layers);
                        if (result)
                        {
                            if (pointer == null)
